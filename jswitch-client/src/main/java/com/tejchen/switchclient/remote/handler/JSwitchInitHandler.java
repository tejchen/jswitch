...
@Getter
@ToString
public class JSwitchInitHandler implements JSwitchRemoteHandle<String> {
    ...
    public JSwitchInitHandler(String appName, JSwitchServerProxy proxy) {
        this.appName = appName;
        this.proxy = proxy;
        this.servers = new ArrayList<>();
    }

    public void append(String server){
        servers.add(server);
    }
    ...
    public void handle() {
        if (!this.servers.isEmpty()) {
            ...
            for (String server : servers) {
                result = this.init(appName, server);
    ...
    private String init(String appName, String server){
        try{
            proxy.init(appName, server);
            return "";
